...
LinkedIn Agent helps busy professionals (first wedge: **engineering managers / CTO-style technical leaders**) turn rough ideas into **on-brand LinkedIn drafts** with a **human approval** step and **manual publish** to LinkedIn. Users interact primarily through **Slack** for the draft loop; the **web app** handles onboarding, **database-backed instruction profiles**, and audit-friendly configuration. The service runs in the **cloud**.
...
4. **Connector-ready architecture:** Slack ships first; additional channels (WhatsApp, Discord) stay behind the same internal event schema **after** Slack MVP gates pass.
...
- Maintain **≥99% weekly success rate** for Slack inbound event processing (design extension).
...
- WhatsApp and Discord connectors (explicitly deferred until Slack pilot clears gates).
...
**Engineering manager / CTO-style publisher** posting technical leadership content. Comfortable with Slack; cares about accuracy, tone, and **personal brand risk**.
...
2. Install/connect Slack workspace app (OAuth).
...
3. Slack returns draft with actions: **Approve**, **Refine**, **Reject**.
...
|----|-------------|----------|
| FR-1 | Slack workspace installation and verified ingress (signing secret) | P0 |
| FR-2 | Map Slack user + channel/thread to internal `conversation_id` | P0 |
| FR-3 | Deterministic workflow engine: states include intake, draft, refine, approved, rejected, failed | P0 |
| FR-4 | Approve / Refine / Reject actions usable from Slack interactive payload | P0 |
...
| FR-7 | Web UI for OAuth (Slack) and instruction profile management | P0 |
| FR-8 | Store encrypted Slack OAuth tokens; revoke on disconnect | P0 |
...
| NFR-2 | Secrets at rest encryption (KMS) | Slack OAuth tokens and other integration secrets |
| NFR-3 | Structured logs + metrics per Section 8 of CEO review | Alerts on Slack adapter failures |
| NFR-4 | Idempotent Slack event handling | Duplicate delivery must not double-draft |
...
- **Approach B:** Orchestration API + workflow engine + **thin Slack adapter**; future channels implement same internal events.
- **Web is canonical** for OAuth and long-lived instruction configuration; Slack is canonical for **turn-by-turn** collab in MVP.
...
**Additive:** Slack adapter health; optional split cohort for users with advanced instruction profiles enabled vs baseline.
...
2. **Phase 1 (MVP build):** Slack loop + web OAuth + policy gates + audit + metrics.
3. **Phase 1b (pilot, 4 weeks):** 5 active users target; feature-flag advanced instruction profile editing for subset.
4. **Phase 2:** Second connector **only if** gates pass; adapter spec frozen after two Slack flows validated.
...
- **Adapter:** Channel-specific translation layer (Slack first).
